...
async function createOrReuseSandbox(daytona: Daytona, sandboxId?: string) {
  ...
  try {
    ...
    console.log(
      `Skipping sandbox reuse (daytona.list() failed: ${error.message}). Creating a new sandbox instead.`
    );
    reusableSandbox = undefined;
  }
  if (reusableSandbox) {
...
async function loadClaudeCode() {
  const mod = require(${JSON.stringify(sdkPath)});
  return mod.query ? mod : (mod.default ?? mod);
}
